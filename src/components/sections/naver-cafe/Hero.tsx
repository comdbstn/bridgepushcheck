...
export function Hero() {
    const { count: cafeCount, ref: cafeRef } = useCountAnimation(3500);
    const { count: userPercentage, ref: percentageRef } = useCountAnimation(98);
                ...
                >
                    <div className='space-y-12 mb-12'>
                        ...
                        <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-8 mt-12 pb-8 sm:pb-0'>
                            {[
                                { number: cafeCount, suffix: "만", label: "월간 활성 유저", ref: cafeRef },
                                { number: userPercentage, suffix: "%", label: "정보 신뢰도", ref: percentageRef },
                                { number: monthlyUsers, suffix: "%", label: "평균 ROI", ref: monthlyRef },
                            ].map((stat, index) => (
                                <motion.div
                                    key={stat.label}
                                    initial={{ opacity: 0, y: 20 }}
